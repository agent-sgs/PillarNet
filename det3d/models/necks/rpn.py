...
class RPNG2(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        assert len(self._layer_strides) == len(self._layer_nums)
        assert len(self._upsample_strides) == len(self._layer_nums)
        assert len(self._num_upsample_filters) == len(self._num_filters)
        ...
        self.deblock_4 = Sequential(
            nn.ZeroPad2d(1),
            nn.Conv2d(self._num_filters[1], self._num_upsample_filters[1], 3, stride=1, bias=False),
        ...
        self.deblock_3 = Sequential(
            nn.ZeroPad2d(1),
            nn.Conv2d(self._num_filters[2], self._num_upsample_filters[2], 3, stride=1, bias=False),
            build_norm_layer(self._norm_cfg, self._num_upsample_filters[2])[1],
            nn.ReLU(),
        )

        self.block_3, num_out_filters = self._make_layer(
            self._num_upsample_filters[2] * 2,
            self._num_upsample_filters[2] * 2,
            self._layer_nums[1],
            stride=1,
        )
